...
export class C05securityComponent implements OnInit {
  securitytype: any = {
    'burglar alarm': {
      ...
      selectedview: this.dataservice.allData.info_details.c05security.indexOf('burglar alarm') > -1
    },
    'sequrity camera': {
      ...
      selectedview: this.dataservice.allData.info_details.c05security.indexOf('sequrity camera') > -1
    }
  };
  ...
  changed(el: any) {
    if (el.checked) this.dataservice.allData.info_details.c05security.push(el.name);
    else {
      let idx  = this.dataservice.allData.info_details.c05security.indexOf(el.name);
      console.log(idx);
      if (idx > -1) this.dataservice.allData.info_details.c05security.splice(idx, 1);
    }
